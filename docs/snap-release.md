...
Use one product `v<version>` tag across platforms, such as `v1.1.0`, and keep Store revision numbers as package-specific build identifiers. The tag-triggered **Release Validation** workflow validates both hosts and retains its inspected Linux Snap as a diagnostic artifact. It does not create a GitHub release automatically or publish a Windows package. After validation succeeds, dispatch **Publish Snap Store Edge** at that exact tag. Attach `openza-tasks-store-snap` from that successful publishing run and its SHA-256 checksum to the GitHub release so the downloadable asset is the same file uploaded to the Store.

If Store review delays channel availability, the verified Snap may be published as a GitHub prerelease with the local-package installation command and a clear explanation that `--dangerous` means the package lacks a Store assertion while remaining strictly confined. After Store approval, promote the same revision unchanged through the accepted channels and update the GitHub release with the Store installation and `snap refresh --amend` transition instructions. Describe which platforms received that version, preserve unpublished Windows drafts, and do not claim Windows availability until the tagged Windows host has passed its separate acceptance checks.
