...
test "$(grep -Fc "version: '0.1.8'" "$manifest")" -eq 1
test "$(grep -Ec "^[[:space:]]+Version: '0.1.8'$" "$manifest")" -eq 2
test "$(grep -Ec "^[[:space:]]+InformationalVersion: '0.1.8'$" "$manifest")" -eq 2
...
grep -Fq '<release version="0.1.8" date="2026-08-29" />' "$repo_root/snap/gui/com.openza.Tasks.metainfo.xml"
