...
test "$(grep -Fc "version: '0.1.6'" "$manifest")" -eq 1
test "$(grep -Ec "^[[:space:]]+Version: '0.1.6'$" "$manifest")" -eq 2
test "$(grep -Ec "^[[:space:]]+InformationalVersion: '0.1.6'$" "$manifest")" -eq 2
...
grep -Fq '<release version="0.1.6" date="2026-08-19" />' "$repo_root/snap/gui/com.openza.Tasks.metainfo.xml"
